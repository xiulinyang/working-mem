...
def alibi_slope_scheduler(initial_slope, final_slope, num_epochs, current_epoch):

    progress = (current_epoch - 1) / (num_epochs - 1)  # `epoch1 = 0.0`, `epoch10 = 1.0`
    return initial_slope * (1 - progress) + final_slope * progress
...
class GPT2Attention(nn.Module):
    ...
    def update_alibi_slope(self):
        ...
        current_slope = alibi_slope_scheduler(
            self.initial_slope,
            self.final_slope,
            self.num_epochs,
            self.current_epoch
        ...
        self.register_buffer(
            "m",
            torch.tensor([1 / (current_slope ** (i + 1)) for i in range(self.num_heads)])
            .unsqueeze(-1)
            .unsqueeze(-1)
        )
